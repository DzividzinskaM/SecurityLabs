...
const hexToDecimal = (hexNumbersArray) =>
  hexNumbersArray.map((item) => parseInt(item, 16));
...
fs.readFile('./task2Cipher.txt', 'utf8', (err, data) => {
  ...
  const hexNumbersArray = data.split('').reduce((acc, item, index, arr) => {
  ...
  const variantsStr = variants.reduce(
    (acc, variant, index) => (acc += `Key ${index}\n` + variant + '\n\n\n'),
    ''
  );
